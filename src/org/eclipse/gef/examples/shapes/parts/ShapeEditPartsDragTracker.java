...
public class ShapeEditPartsDragTracker extends SelectEditPartTracker {
	...
	private Insets threshold = new Insets(18);
	...
	protected void doAutoexpose(){
		ShapeEditPart part = ((ShapeEditPart)getSourceEditPart());
		Point where = getLocation();
		Viewport port = findViewport((GraphicalEditPart)part.getParent().getParent());
		...
		Point shapeloc = part.getCastedModel().getLocation();
		int x = shapeloc.x, y = shapeloc.y;
		if (oldloc.x <= loc.x){
			automove=true;
			x = loc.x;
		}
		...
		if (automove)
			part.getCastedModel().setLocation(new Point(x, y));
