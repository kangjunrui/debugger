...
import debugger.shapes.ShapesViewport;


/**
 * A DragTracker that moves {@link org.eclipse.gef.EditPart EditParts}.
 */
public class ShapeEditPartsDragTracker extends SelectEditPartTracker {
	public static class ShapeMoveAutoexposeHelper implements AutoexposeHelper {
		...
		public ShapeEditPart part;
		
		protected Viewport findViewport(GraphicalEditPart p) {
			IFigure figure = null;
			port = null;
			do {
				if (figure == null)
					figure = p.getContentPane();
				else
			...
			} while (figure != p.getFigure() && figure != null);
			return port;
		}


		public ShapeMoveAutoexposeHelper(GraphicalEditPart owner) {
			this.owner = owner;
			this.part= ((ShapeEditPart)owner);
			threshold = DEFAULT_EXPOSE_THRESHOLD;
			findViewport((GraphicalEditPart)owner.getParent().getParent());
		}
		...
		public boolean step(Point where) {
			...
			Dimension size = port.getBounds().getSize();
			rect.translate(loc);
			rect.width=rect.x+rect.width;
			rect.height=rect.y+rect.height;
			rect.x=0;
			rect.y=0;
			boolean templarger=false;
			...
			if (templarger){
				...
				((ShapesViewport)port).setTempXY(rect.width, rect.height);
			}
	...
	private ShapeMoveAutoexposeHelper exposeHelper;
	 ...
	 */
	public ShapeEditPartsDragTracker(EditPart sourceEditPart) {
		super(sourceEditPart);
		exposeHelper = new ShapeMoveAutoexposeHelper((GraphicalEditPart)sourceEditPart);
	...
	public void deactivate() {
		isAutoexpose = false;
		((ShapesViewport)exposeHelper.port).restoreXY();
		eraseSourceFeedback();
		super.deactivate();
		exclusionSet = null;
