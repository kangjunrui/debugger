...
import debugger.shapes.model.commands.ConnectionCreateCommand;
import debugger.shapes.model.commands.ConnectionReconnectCommand;
...
public class ShapeEditPart extends AbstractGraphicalEditPart implements
	...
	public ShapeEditPart(ShapesEditor editor) {
		this.editor=editor;
	}
	...
	public void activate() {
		if (!isActive()) {
			...
			getFigure().addMouseListener(new MouseListener(){

				@Override
				public void mousePressed(MouseEvent me) {
					if ((me.getState()&SWT.SHIFT)!=0){
						List conns = getTargetConnections();
						Object[] parts = new Object[conns.size()];
						int count=0;
						for (Object conn : conns){
							parts[count]=((AbstractConnectionEditPart)conn).getSource();
							count++;
						}
	...
	Label m_name;
	ShapeFigure actualFigure=null;
	/**
	 * Return a IFigure depending on the instance of the current model element.
	 * This allows this EditPart to be used for both sublasses of Shape.
	 */
	private IFigure createFigureForModel() {
		actualFigure = new ShapeFigure(getCastedModel().getType());
		...
		if (getCastedModel().getIcon()!=-1)
			m_name.setIcon(editor.getImage(getCastedModel().getIcon()));
	...
	public boolean setShowfilename(boolean state){
		...
		if (state && getCastedModel().resource==-1)
			return oldstate;
		if (state){
			String name=getCastedModel().diagram.getFile(getCastedModel().resource);
	...
	protected ConnectionAnchor getConnectionAnchor() {
		if (anchor == null) {
			if (getCastedModel().getType()==1)
				anchor = new EllipseAnchor(getFigure());
			else if (getCastedModel().getType()==0)
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		} else if (Shape.TYPE_PROP.equals(prop)) {
			actualFigure.setType(getCastedModel().getType());
		}
	}
	...
	public void performRequest(Request request) {
		if (request.getType() == RequestConstants.REQ_DIRECT_EDIT) {
			...
			Shape shape = getCastedModel();
			if (shape.resource!=-1 && shape.getIcon()<9){
				//see line and signature to get offset, if no offset (-1)
				//then do not bringToTop and selectAndReview
				if (!editor.uiutil.setEditorSelection(shape)){
					editor.uiutil.setExplorerSelection(shape);
				}
			}else if (shape.resource!=-1){
				//if icon is for setEditorSelection, only activate editor.
				editor.uiutil.setExplorerSelection(shape);
			}
		}
	}
